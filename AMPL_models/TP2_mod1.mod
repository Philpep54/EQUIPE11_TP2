# Technique de résolution 1
# Scénario où nous ouvrons une nouvelle usine et nous avons aucune contrainte de longueur et de largeur d'usine
...
subject to dist_horizontal {i in K, j in K: 1<= i < j <= n}: 
	dx[i,j]>= X[i]-X[j]; #Établissement distance horizontale entre les départements
